...
def calc_vehicle_offset(undist, left_fit, right_fit):
    ...
    bottom_x_left = left_fit[0] * (bottom_y ** 2) + \
                    left_fit[1] * bottom_y + left_fit[2]
...
def final_viz(undist, left_fit, right_fit, m_inv, left_curve, right_curve, vehicle_offset):
    ...
    # result = cv2.putText(result, label_str, (30, 40), 0,
    #                      0.8, (0, 255, 0), 1, cv2.QT_FONT_NORMAL)

    label_str = 'Vehicle offset from lane center: %.1f m' % vehicle_offset
    result = cv2.putText(result, label_str, (30, 70), 0,
                         0.4, (0, 255, 0), 1, cv2.QT_FONT_NORMAL)
