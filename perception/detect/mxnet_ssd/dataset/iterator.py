...
class ImageIter(mx.io.DataIter):
    ...
    def __init__(self, img, data_shape, mean_pixels, batch_size=1):
    ...
    def _get_batch(self):
        ...
        if self.current_batch >= 1:
            print('==== now is time to stop, we are have batch for {}'.format(self.current_batch))
            return False
        else:
            batch_data = mx.nd.zeros((self.batch_size, 3, self._data_shape[0], self._data_shape[1]))
            for i in range(self.batch_size):
                ...
                batch_data[i] = self.pre_process_data(mx.nd.array(img))
            self._data = {'data': batch_data}
            self._label = {'label': None}
            return True
